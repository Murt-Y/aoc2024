...
def Checkup(list):
    ...
    for l in list:
        newi=l-(xmax2+1)
        if(coordinates2[newi].val=='['):
            newlist.append(newi)
            newlist.append(newi+1)
        elif(coordinates2[newi].val==']'):
            newlist.append(newi)
            newlist.append(newi-1)
        elif(coordinates2[newi].val=='#'):
            return False
    return Checkup(newlist)
...
for c in instructions:
    ...
    if(c=='^'):
        if Checkup([robotindex]):
            PushUp2(robotindex)
    elif(c=='v'):
        if Checkdown([robotindex]):
